      ...
      User.findOne({ email: req.params.email }, (err, user) => {
        if (err) {
          ...
          if (user) {
            ...
            res.json({ success: true, message: 'E-mail is ok' }); // Return as available e-mail
          }
        }
      });
    }
      ...
      User.findOne({ username: req.params.username }, (err, user) => { // Check if connection error was found
        if (err) {
          ...
          if (user) {
            ...
            res.json({ success: true, message: 'Username is ok' }); // Return as vailable username
          }
        }
      });
    }
          ...
          if (err) {
            ...
            if (!user) {
              ...
              if (!validPassword) {
                ...
                // res.send("logged");
                // console.log("logged");
                const token1 = jwt.sign({ userId: user._id }, config.secret, { expiresIn: '24h' }); // Create a token for client
                console.log(token1);
                console.log(user);
                res.json({
                  success: true,
